...
const Chats = () => {
  ...
  return (
    <>
    <NavBar/>
    <div class="min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8" style={{ background: 'linear-gradient(to top, #003366 0%, #ffffff 100%)' }}>

    <div className="w-full max-w-lg">
        <div className='justify-center text-center' >
            <h1 className='font-semibold text-[#fff] text-xl'>You have previously chatted with :</h1>
        </div>
        <div className="m-4 sm:m-8 space-y-4">
            {chatEmails.map((email, index) => (
                <div key={index} class="p-4 bg-black rounded-lg flex flex-col items-center sm:flex-row justify-between space-y-2 sm:space-y-0 space-x-0 sm:space-x-4">
                    <div class="w-full sm:w-fit bg-gray-300 rounded">
                        <h2 class="p-2 text-[#222]">{email}</h2>
                    </div>
                    <div className="mt-2 sm:mt-0 w-full sm:w-auto h-10 rounded-lg bg-purple-300 hover:bg-purple-700 flex justify-center">
